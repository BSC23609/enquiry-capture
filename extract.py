...
KW_PRODUCT = [
    ...
    # steel-processing / cutting-plan vocabulary — real orders specify these
    "cutting plan", "cut length", "cut to length", "ctl", "shearing", "slitting",
    "grade", "thickness", "gauge", "gsm", "hrpo", "gi sheet", "gp sheet",
    "gc sheet", "chequered", "flat", "round bar", "square bar", "pipe", "tube",
    "raw material", "sheet metal", "ms sheet", "ms angle", "ms channel",
]
KW_INTENT = [
    ...
    # order/planning language a buyer uses when placing or scheduling material
    "kindly plan", "place the vehicle", "confirm date", "required qty",
    "required quantity", "please plan", "delivery", "dispatch", "order",
]


# Some mails are unmistakably an order/plan even without a separate product
# word — a cutting plan naming grade+thickness+width+length is a firm
# requirement. If several of these structural terms co-occur, treat it as a
# product signal on its own.
KW_SPEC = ["grade", "thickness", "width", "length", "qty", "quantity",
           "vendor code", "customer code", "cutting plan"]
...
def prefilter(subject: str, body: str, from_addr: str) -> tuple[bool, str]:
    ...
    # A cutting plan / material spec (grade + thickness + width + length + qty)
    # is a firm requirement even when no product noun appears. If 3+ spec terms
    # co-occur, count it as a product signal so these orders aren't dropped.
    spec = sum(1 for k in KW_SPEC if k in hay)
    if spec >= 3:
    ...
    if product == 0 or intent == 0:
        return False, f"no product+intent signal (p={product}, i={intent}, spec={spec})"
